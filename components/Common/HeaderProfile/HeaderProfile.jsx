            ...
            <div className="flex gap-5 items-center">
                ...
                <div>
                    <h2 className="font-normal">
                        Hello <span className="font-semibold">{user.role === 'admin' ? 'Admin Name' : user.role === 'coach' ? 'Staff Name' : 'Faisal'}</span>
                    </h2>
                    <p className={`text-zinc-400 text-sm ${(user.role === 'coach' || user.role === 'admin') && 'hidden'}`}>
                        Credits Available: <span className="text-primary">10</span>
                    </p>
                </div>
            </div>
            <div className={`flex space-x-4 ${(user.role === 'coach' || user.role === 'admin') && 'hidden'}`}>
