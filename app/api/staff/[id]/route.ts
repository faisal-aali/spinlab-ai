...
export async function POST(req: NextRequest) {
    try {
        ...
        const id = pathname.split('/').pop(); // /api/staff/:id
        if (!id) return NextResponse.json({ message: 'id is required' }, { status: 400 })

        if (session.user.role !== 'admin' && id !== session.user._id) return NextResponse.json({ message: 'Unauthorized' }, { status: 401 })

        const user = await User.findOne({ _id: id, role: 'staff', isDeleted: false })
        if (!user) return NextResponse.json({ message: 'Staff does not exist' }, { status: 400 })
        ...
        return NextResponse.json({ message: `Staff has been updated` }, { status: 200 })
    } catch (err: unknown) {
        console.error(err)
...
export async function DELETE(req: NextRequest) {
    try {
        ...
        return NextResponse.json({ message: 'Staff has been deleted' }, { status: 200 })
    } catch (err: unknown) {
        console.error(err)
        const obj = validateError(err)
        return NextResponse.json({ message: obj.message }, { status: obj.status })
