...
export async function POST(req: NextRequest) {
    try {
        ...
        const id = pathname.split('/').pop(); // /api/admin/:id
        ...
        if (!user) return NextResponse.json({ message: 'Admin does not exist' }, { status: 400 })
        ...
        return NextResponse.json({ message: `Admin has been updated` }, { status: 200 })
    } catch (err: unknown) {
        console.error(err)
